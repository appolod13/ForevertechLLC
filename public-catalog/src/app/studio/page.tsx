...
function StudioPageInner() {
  ...
  const generateImage = async () => {
    ...
    try {
      ...
      // === VERY ROBUST BASE64 HANDLING ===
      let imageUrl = '';

      if (data.image_data_url && typeof data.image_data_url === 'string') {
      ...
      if (!imageUrl) {
        console.error("Full API response received:", data);
        throw new Error('No valid image data returned from server');
      }
      ...
      alert(error.message || 'Generation failed. Check console for details.');
    } finally {
                ...
                <div className="aspect-video relative bg-zinc-950">
                  {generatedImage ? (
                    ...
                    <div className="absolute inset-0 flex flex-col items-center justify-center text-center px-8">
                      ...
                      <p className="text-gray-400 text-2xl">Your fractal will appear here</p>
                    </div>
                  )}
                ...
                {generatedImage && (
                  <div className="bg-zinc-950/95 px-5 py-3 flex items-center justify-between text-sm border-t border-gray-800">
                    ...
                    <button
                      ...
                      className="px-4 py-1.5 rounded-xl border border-gray-700 text-xs flex items-center gap-2 hover:bg-gray-900 transition"
                    >
                      ⬇ Download
                    </button>
                  </div>
