...
type PosterPlatformKey = 'reddit' | 'discord' | 'rss';
...
const POSTER_PLATFORM_NAMES: Record<PosterPlatformKey, string> = {
  reddit: 'Reddit',
  discord: 'Discord',
  rss: 'RSS',
};
...
function buildPosterPlatformStates(value: unknown): Record<PosterPlatformKey, PosterPlatformState> {
...
function StudioPageInner() {
  ...
  const posterMediaUrl = useMemo(() => previewImageUrl || generatedImage || latestDropImageUrl || '', [previewImageUrl, generatedImage, latestDropImageUrl]);
  const posterDefaultCopy = useMemo(() => {
    const generated = generatedTextContent.trim();
    if (generated) return generated;
    return prompt.trim();
  }, [generatedTextContent, prompt]);
            ...
            <div className="mt-8 border-t border-gray-700 pt-8">
              ...
              <div className="mt-8 rounded-xl border border-gray-700 bg-gray-950/60 p-5">
                <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                  <div>
                    <h3 className="text-xl font-bold text-white">Multichannel Poster</h3>
                    <div className="mt-1 text-sm text-gray-400">
                      Use the latest generated image and caption to post across connected channels.
                    </div>
                  </div>
                  <button
                    type="button"
                  ...
                  {(['reddit', 'discord', 'rss'] as PosterPlatformKey[]).map((platform) => (
                    <label
                      ...
                      className={`rounded-lg border p-3 transition-all ${
                        posterConnections[platform].authenticated ? 'border-gray-700 bg-gray-900/80 hover:border-gray-500 cursor-pointer' : 'border-gray-800 bg-gray-900/40 opacity-80 cursor-not-allowed'
                      }`}
                    >
                      <div className="flex items-start gap-3">
                        <input
