 ...
 * NOW SENDS ACTUAL RENDER PARAMETERS to Fusion API for visual diversity
 */
async function tryFusionGenerate(
  ...
  try {
    ...
    let renderParams = {}; // NEW: Store render parameters
    let renderParamsStr = ""; // NEW: Query string format
    ...
    if (use_fractal_fusion) {
      ...
      renderParams = fractalFusion.render_params; // NEW: Capture render parameters
      renderParamsStr = fractalFusion.api_params; // NEW: Capture as query string
      ...
      logInfo("fractal_fusion.applied", {
        ...
        fractal_types: fractalFusion.fractal_config.secondary_fractals,
        primary_fractal: fractalFusion.fractal_config.primary_fractal,
        emotion: fractalFusion.fractal_config.emotion,
        complexity: fractalFusion.fractal_config.complexity,
        intensity: fractalFusion.fractal_config.intensity,
    ...
    // === NEW: Build request body with render parameters ===
    ...
    // NEW: Include render parameters if available
    if (Object.keys(renderParams).length > 0) {
      requestBody.fractal_render_params = renderParams;
      requestBody.fractal_config = {
        use_multi_fractal: true,
        render_params_included: true,
      };
    }

    // NEW: If query string format is needed, also include it
    if (renderParamsStr) {
      requestBody.fractal_render_query = renderParamsStr;
    }
    ...
    if (d.success === true && typeof d.imageUrl === "string") {
      ...
      return {
        ...
        meta: isRecord(d.meta)
          ...
          : { provider: "fusion", fractal_fusion_enabled: use_fractal_fusion, render_params_applied: Object.keys(renderParams).length > 0 },
      };
    }
    return null;
  } catch (e) {
