...
function drawFuturisticLines(params: {
  ...
  const pad =
    variant === "abstract"
      ? Math.max(95, Math.round(Math.min(bgW, bgH) * 0.105))
  ...
  const baseR = Math.min(w, h) * (variant === "abstract" ? 0.39 + rng() * 0.07 : 0.28 + rng() * 0.06);
  ...
  for (let i = 0; i < nodeCount; i++) {
    ...
    const squish = variant === "abstract" ? 1.28 + rng() * 0.24 : 0.88 + rng() * 0.28;
  ...
  if (variant === "abstract") {
    ...
    const pad = Math.max(112, Math.round(Math.min(bgW, bgH) * 0.11));
    const x0 = bgX + pad;
    const y0 = bgY + pad;
    const w = bgW - pad * 2;
    const h = bgH - pad * 2;
    ...
    const size = Math.min(w, h) * (0.50 + orng() * 0.06);
    const depth = size * (0.20 + orng() * 0.04);
