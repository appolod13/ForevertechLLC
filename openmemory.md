...
- Printify back print: src/lib/printifyBackText.ts renders the back panel as a PNG via @napi-rs/canvas and route handlers composite the QR stamp via sharp. Modes: words collage or abstract scute-style mesh line art (darker + larger geometry) with thick black strokes plus a large centered “diamond/hypercube” wireframe overlay; currently the rendered back panel does not draw the word tiles (to avoid crowding). Back includes a fixed top header “Pixel Crypted” scaled to near edge-to-edge and positioned flush near the top edge of the red panel. QR stamp includes a QV square badge + “Quantum Verified” + https://www.pixelqrypt.com (no “Pixel Crypted” line on the stamp). Controlled by env PRINTIFY_BACK_STYLE=words|abstract (default abstract); admin sample API accepts backStyle too.
