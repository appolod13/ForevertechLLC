...
- Back customization: ProductCustomizer includes an optional customer back text field (stored on cart item metadata as backCustomerText). /api/back-preview overlays it on the back panel for preview, and /api/stripe/webhook includes it when generating the uploaded Printify back image (without changing the abstract pattern seed).
