...
- Cart UX (client): CartContext is localStorage-first for reliability on Vercel serverless; add/remove/clear update local state immediately and only best-effort sync to /api/cart/* (in-memory). Checkout always posts the current cart items in the request body. /checkout/success clears the local cart on load.
