...
- SEO metadata: src/app/layout.tsx sets metadataBase, title template, description, keywords, Open Graph, and Twitter card tags; key policy pages also include SEO titles/descriptions/keywords via per-page metadata exports.
...
- Checkout + fulfillment: /checkout UI calls /api/checkout (Stripe session) → /api/stripe/webhook (creates Printify order + uploads assets). /api/checkout accepts cart items in the request body and persists an order snapshot to Supabase (orders + order_items) keyed by stripe_checkout_session_id; /api/stripe/webhook loads those items from Supabase first (falls back to in-memory cartStore only if Supabase snapshot is unavailable). Server routes use SUPABASE_SERVICE_ROLE_KEY when present, otherwise fall back to NEXT_PUBLIC_SUPABASE_ANON_KEY (requires RLS policies to permit the inserts/selects). Checkout supports optional customer-provided qrUrl (stored in Stripe metadata) and a qrDisabled flag (Stripe metadata qrDisabled='1') to remove the QR stamp entirely. Admin sample endpoint /api/admin/printify-back-text also accepts qrUrl for preview/product samples; admin UI requests SVG previews via POST {previewSvg:true} to avoid bundling server-only canvas code into the client.
- Checkout QR link field: /checkout includes a “QR Link (optional)” input (example https://yourbusiness.com) placed at the bottom of the shipping address section; if provided, it is encoded into the QR stamp on the back and stored in Stripe metadata.
- Customize QR link field: /customize (ProductCustomizer) includes a “QR Link (optional)” input plus an Include QR stamp toggle; if enabled, qrUrl updates the back preview QR stamp target and is saved into cart item metadata.qrUrl. If disabled, it sets metadata.qrDisabled=true and /api/back-preview and /api/stripe/webhook omit the QR stamp.
- Cart UX (client): CartContext is localStorage-first for reliability on Vercel serverless; add/remove/clear update local state immediately and only best-effort sync to /api/cart/* (in-memory). Stored cart items are normalized on load (quantity clamped; metadata.productId='tee'; size inferred and metadata.variant set) so legacy carts can checkout. Checkout always posts the current cart items in the request body. /checkout/success clears the local cart on load.
- Live status: Providers open an EventSource connection to /api/events (SSE ping/new_post) for the Live badge. This is not checkout; it’s a long-lived stream request that appears in DevTools. Live status is disabled on /checkout* routes to reduce Network noise during payment troubleshooting.
- Global navigation arrows: Providers render fixed Back/Forward buttons (uses browser history) so users can navigate even if the browser UI is missing; intended to help especially on /checkout.
- Back customization: ProductCustomizer back text supports draft + Done/Enter apply; preview can reflect draft text and stores backCustomerText for orders. /api/back-preview overlays it at the top of the back panel for preview, and /api/stripe/webhook includes it when generating the uploaded Printify back image (without changing the abstract pattern seed). Overlay scales horizontally to span the red panel width and slightly stretches vertically; max length 64 chars.
...
- Client API bases: NEXT_PUBLIC_MIRROR_API_URL and NEXT_PUBLIC_EVENTS_URL are treated as optional overrides; invalid/non-https/localhost values are ignored in production so the app falls back to same-origin (/api/*) instead of trying to hit malformed URLs.
- Auth requests: AuthContext uses same-origin /api/auth/login and /api/auth/register (not NEXT_PUBLIC_MIRROR_API_URL) to avoid cross-origin/CORS failures that appear as “Failed to fetch” during signup/login.
...
- Home hero SEO copy: the Latest Build headline starts with “Math Art” and includes emotional/prompt and Mandelbrot + Julia fractal wording for search relevance.
...
- Printify back print: src/lib/printifyBackText.ts renders the back panel as a PNG via @napi-rs/canvas and route handlers composite the QR stamp via sharp/qrcode. Modes: words collage or abstract scute-style mesh line art tuned to a medium size (more padding than original, but larger than “fully shrunk”) with thick black strokes plus a centered wireframe overlay. Back no longer includes a fixed top header; customer-entered back text (if provided) is overlaid at the top by /api/back-preview and /api/stripe/webhook when creating Printify back images. QR stamp includes “Quantum Verified” + www.pixelqrypt.com (no “Pixel Crypted” line and no QV badge on the stamp); labels are placed below the QR (two lines) and stretched horizontally to fit corner-to-corner. Stamp background is fully opaque and routes wipe the pattern behind the stamp area before compositing, so the QR never gets visually blocked. All stamp + header/customer text uses a custom vector stroke font drawn on canvas (no system font dependency) so it renders reliably on Vercel.
- Customizer back preview: /api/back-preview renders the back print PNG server-side (customer text overlay + optional QR stamp) so the Customize page matches Printify output and stays clipped inside the red panel.
...
- Header navigation (desktop): header tabs are collapsed into a single “Navigate” dropdown to reduce clutter; Admin appears only when allowed; Tools is always visible; Scanner is dev/admin only.
- Stripe compliance mode: customer-facing crypto payments + NFT claim UI are disabled; crypto/NFT endpoints return 404 unless ENABLE_CRYPTO_PAYMENTS=1 / ENABLE_NFT_CLAIM=1 are explicitly set.
- Stripe copy hygiene: avoid “crypto/NFT/mint/gasless” wording in customer-facing pages; PixelQrypt is described as a verification code + download access (Stripe “digital download access”), not a crypto/NFT feature.
- Customer policy pages: /terms, /refund-policy, /shipping-policy exist for Stripe review; global footer links to these pages + Support + Privacy.
- Support contact info: Support email support@forevertech.tech; address 84 Luisa St, Brooklyn, NY 10223; hours 24/7 (shown in Support page + footer).
- Auth (Supabase): /api/auth/login and /api/auth/register provide email+password login/signup via Supabase Auth and match the frontend AuthContext response shape ({success, user}). Registration uses SUPABASE_SERVICE_ROLE_KEY for instant confirmed users when available, otherwise uses anon signUp.
...
- Customer education: FAQs page at /faqs explains Studio → Customize → Cart → Checkout → Printify plus Quantum Verified Premium; links to policy pages (shipping/refunds).
