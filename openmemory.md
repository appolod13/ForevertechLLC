...
- Back customization: ProductCustomizer includes an optional customer back text field with a Done action (stored on cart item metadata as backCustomerText). /api/back-preview overlays it at the top of the back panel for preview, and /api/stripe/webhook includes it when generating the uploaded Printify back image (without changing the abstract pattern seed). The overlay scales horizontally to span the red panel width.
