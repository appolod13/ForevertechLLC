...
- Printify back print: src/lib/printifyBackText.ts renders the back panel as a PNG via @napi-rs/canvas and route handlers composite the QR stamp via sharp/qrcode. Modes: words collage or abstract scute-style mesh line art (darker + larger geometry) with thick black strokes plus a large centered “diamond/hypercube” wireframe overlay; currently the rendered back panel does not draw the word tiles (to avoid crowding). Back no longer includes a fixed top header; customer-entered back text (if provided) is overlaid at the top by /api/back-preview and /api/stripe/webhook when creating Printify back images. QR stamp includes “Quantum Verified” + www.pixelqrypt.com (no “Pixel Crypted” line and no QV badge on the stamp); labels are placed below the QR (two lines) and stretched horizontally to fit corner-to-corner. Stamp labels use a custom vector stroke font drawn on canvas (no system font dependency) so they render reliably on Vercel. Controlled by env PRINTIFY_BACK_STYLE=words|abstract (default abstract); admin sample API accepts backStyle too.
